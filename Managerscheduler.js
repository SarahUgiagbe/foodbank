...
/* THE FOLLOWING HAS BEEN ADDED BY THAURA */
...
function openScheduleModal(dateStr) {
    ...
    // Load staff data for this date
    loadStaffSchedule(dateStr);
}
...
function loadStaffSchedule(dateStr) {
    ...
    setTimeout(() => {
        const staffData = getSampleStaff(dateStr);
        
        let html = '<h4>Staff Working</h4>';
        html += '<ul class="list-group">';
        staffData.working.forEach(person => {
            html += `<li class="list-group-item">${person.name} - ${person.role}</li>`;
        });
        html += '</ul>';
        
        html += '<h4 class="mt-3">Staff Requests</h4>';
        html += '<ul class="list-group">';
        staffData.requests.forEach(person => {
            html += `<li class="list-group-item">${person.name} - Requested shift</li>`;
        });
        html += '</ul>';
...
function getSampleStaff(dateStr) {
    // Return sample data - replace with your actual data
    return {
        working: [
            {name: 'John Smith', role: 'Manager'},
            {name: 'Sarah Johnson', role: 'Volunteer Coordinator'},
            {name: 'Mike Davis', role: 'Driver'}
        ],
        requests: [
            {name: 'Emma Wilson'},
            {name: 'James Brown'}
